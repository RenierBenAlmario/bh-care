-- Fix doctor weekend availability
-- This script will enable weekend work for all doctors

-- First, check if DoctorAvailabilities table exists and has data
SELECT 'Current DoctorAvailability records:' as Info;
...
-- Update existing DoctorAvailability records to enable weekends
...
-- If no DoctorAvailability records exist, create them for all doctors
...
-- Show which doctors we updated
SELECT 'Doctors with weekend availability:' as Info;
SELECT u.UserName, u.Email, da.Saturday, da.Sunday, da.StartTime, da.EndTime
FROM AspNetUsers u
