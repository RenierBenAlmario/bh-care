@echo off
echo ===================================================
echo Enhanced TimeSpan Error Fixer for Barangay Project
echo ===================================================
echo.
...
if exist "Controllers\Api\DoctorsController.cs" (
    ...
    powershell -Command "$content = Get-Content 'Controllers\Api\DoctorsController.cs' -Raw; $content = $content -replace 's\.MaxDailyPatients\.HasValue\(\)', 's.MaxDailyPatients != null'; $content = $content -replace 'a\.AppointmentTime\.HasValue\(\)', 'a.AppointmentTime != null'; $content = $content -replace 'a\.AppointmentTime\.Value', 'a.AppointmentTime.Value'; Set-Content 'Controllers\Api\DoctorsController.cs' $content"
)
...
if exist "Controllers\DoctorsController.cs" (
    ...
    powershell -Command "$content = Get-Content 'Controllers\DoctorsController.cs' -Raw; $content = $content -replace 'appointment\.AppointmentTime == \"', 'appointment.AppointmentTime.ToString() == \"'; $content = $content -replace '(model\.AppointmentTime = )TimeSpan\.Parse\(([^)]+)\)', '$1TimeSpan.Parse($2)'; Set-Content 'Controllers\DoctorsController.cs' $content"
)
...
if exist "Pages\User\UserDashboard.cshtml" (
    ...
    powershell -Command "$content = Get-Content 'Pages\User\UserDashboard.cshtml' -Raw; $content = $content -replace 'a\.AppointmentTime\.HasValue\(\)', 'a.AppointmentTime != null'; $content = $content -replace 'a\.AppointmentTime\.Value', 'a.AppointmentTime'; $content = $content -replace '(a\.AppointmentTime\.ToString\()\"([^\"]+)\"\)', '$1\"$2\", null)'; Set-Content 'Pages\User\UserDashboard.cshtml' $content"
)
...
if exist "Pages\Admin\AdminDashboard.cshtml.cs" (
    ...
    powershell -Command "$content = Get-Content 'Pages\Admin\AdminDashboard.cshtml.cs' -Raw; $content = $content -replace 'AppointmentTime = a\.AppointmentTime \?\? TimeSpan\.Zero', 'AppointmentTime = a.AppointmentTime ?? TimeSpan.Zero'; Set-Content 'Pages\Admin\AdminDashboard.cshtml.cs' $content"
)
...
if errorlevel 1 (
    ...
    powershell -Command "$errors = Get-Content 'build_errors.txt'; $needsRebuild = $false; foreach ($error in $errors) { if ($error -match '(.*\.cs)\((\d+),(\d+)\).*') { $file = $matches[1]; $line = [int]$matches[2]; if ($error -match 'Cannot implicitly convert type') { echo \"Fixing conversion error in $file at line $line\"; $content = Get-Content $file; if ($content[$line-1] -match 'AppointmentTime\s*=\s*([^;]+)') { $content[$line-1] = $content[$line-1] -replace 'AppointmentTime\s*=\s*([^;]+)', 'AppointmentTime = $1 != null ? $1 : null'; Set-Content $file $content; $needsRebuild = $true; } } elseif ($error -match 'Operator.*cannot be applied to operand of type') { echo \"Fixing operator error in $file at line $line\"; $content = Get-Content $file; if ($content[$line-1] -match '\.AppointmentTime\s*([=!<>]+)\s*') { $content[$line-1] = $content[$line-1] -replace '\.AppointmentTime\s*([=!<>]+)\s*', '.AppointmentTime?.ToString() $1 '; Set-Content $file $content; $needsRebuild = $true; } } } } if ($needsRebuild) { echo \"Additional fixes applied. Attempting rebuild...\"; } else { echo \"Could not automatically fix all errors. Manual intervention required.\"; }"
    
    :: Try to fix DbContext model issues
    if exist "Data\ApplicationDbContext.cs" (
        echo Checking and fixing ApplicationDbContext.cs...
        copy "Data\ApplicationDbContext.cs" "%backupdir%\ApplicationDbContext.cs"
        powershell -Command "$content = Get-Content 'Data\ApplicationDbContext.cs' -Raw; $modified = $false; if ($content -match 'modelBuilder\.Entity<Appointment>\(\)') { $content = $content -replace '(modelBuilder\.Entity<Appointment>\(\)[^}]*)(\.Property\(a => a\.AppointmentTime\)[^;]*;)', '$1'; $modified = $true; } if ($modified) { Set-Content 'Data\ApplicationDbContext.cs' $content; echo \"Fixed ApplicationDbContext configuration for TimeSpan properties.\"; }"
    )
